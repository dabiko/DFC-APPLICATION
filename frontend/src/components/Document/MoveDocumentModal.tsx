 ...
 * Modal for moving documents to a different folder
 */
...
export const MoveDocumentModal: FC<MoveDocumentModalProps> = ({
  ...
  const validateMove = (): string | null => {
    ...
    // For shortcuts, we don't allow move - they should be deleted and recreated
    if (item.isShortcut) {
      return 'Shortcuts cannot be moved. Delete and create a new shortcut instead.'
    }
    ...
    if (selectedFolderId) {
      ...
      if (targetFolder && !targetFolder.permissions.canEdit) {
        return 'You do not have permission to add files to the selected location'
      }
    }
